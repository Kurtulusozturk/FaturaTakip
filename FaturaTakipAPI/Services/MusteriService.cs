...
namespace FaturaTakipAPI.Services
{
	public interface IMusteriService
	{
		...
		List<MusteriGetIdAndFullNameModel> GetMusterilerForDropdown(int id);
	...
	public class MusteriService : IMusteriService
	{
		...
		public List<MusteriGetIdAndFullNameModel> GetMusterilerForDropdown(int id)
		{
			var musterilerList = new List<MusteriGetIdAndFullNameModel>();
			foreach (var item in _dbContext.Musteriler.Include(s => s.Sirket).ToList())
			{
				if (item.Sirket.SirketID == id)
				{
					var selectedMusteri = new MusteriGetIdAndFullNameModel
					{
						MusteriID = item.MusteriID,
						MusteriFullName = item.Ad + " " + item.Soyad
					};
					musterilerList.Add(selectedMusteri);
				}
			}
			return musterilerList;
		}
